...
from verl.models.deepseekv2.megatron.layers import ParallelDeepseekV2DecoderLayer, ParallelDeepseekV2RMSNorm, ParallelDeepseekV2DecoderLayerRmPad
"""
TODO: 
1. Add weight initialization. Here we need to be careful on TP weight init.
2. Add sequence parallel
3. Load checkpoint from meta DeepseekV2 pretrained checkpoint
"""
...
class ParallelDeepseekV2Model(nn.Module):
    """
    ...
    def __init__(self, config: DeepseekV2Config, megatron_config: ModelParallelConfig):
        ...
        self.layers = nn.ModuleList(
            [
                ParallelDeepseekV2DecoderLayer(config, megatron_config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self.norm = ParallelDeepseekV2RMSNorm(config.hidden_size, megatron_config, eps=config.rms_norm_eps)
...
class ParallelDeepseekV2ForCausalLM(nn.Module):
    def __init__(self, config: DeepseekV2Config, megatron_config: ModelParallelConfig):
        super().__init__()
        self.config: TransformerConfig = convert_deepseekv2_config(config, megatron_config)
        self.model = ParallelDeepseekV2Model(config, megatron_config=megatron_config)
        self.vocab_size = config.vocab_size
...
class ParallelDeepseekV2ModelRmPad(nn.Module):
    ...
    def __init__(self, config: DeepseekV2Config, megatron_config: ModelParallelConfig):
        ...
        self.layers = nn.ModuleList(
            [
                ParallelDeepseekV2DecoderLayerRmPad(config, megatron_config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = ParallelDeepseekV2RMSNorm(config.hidden_size, megatron_config, eps=config.rms_norm_eps)
...
class ParallelDeepseekV2ForCausalLMRmPad(nn.Module):

    def __init__(self, config: DeepseekV2Config, megatron_config: ModelParallelConfig):
        super().__init__()
        self.config: TransformerConfig = convert_deepseekv2_config(config, megatron_config)
        self.megatron_config = megatron_config
        self.model = ParallelDeepseekV2ModelRmPad(config, megatron_config=megatron_config)
        self.vocab_size = config.vocab_size
...
class ParallelDeepseekV2ForValueRmPad(ParallelDeepseekV2ForCausalLMRmPad):
...
class ParallelDeepseekV2ModelRmPadPP(nn.Module):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`DeepseekV2DecoderLayer`]
    ...
    def __init__(self, config: DeepseekV2Config, megatron_config: ModelParallelConfig, pre_process, post_process):
        ...
        for i in range(self.num_layer_this_model):
            layer = ParallelDeepseekV2DecoderLayerRmPad(config, megatron_config, i + layer_offset)
...
class ParallelDeepseekV2ForCausalLMRmPadPP(nn.Module):
    ...
    def __init__(self, config: DeepseekV2Config, megatron_config: ModelParallelConfig, pre_process, post_process,
        ...
        self.config: TransformerConfig = convert_deepseekv2_config(config, megatron_config)
        self.megatron_config = megatron_config
        self.model = ParallelDeepseekV2ModelRmPadPP(config,
    ...
    def forward(
        ...
        *,
        ...
        # if torch.distributed.get_rank() == 0:
            # cu_seqlens:tensor([  0, 464, 845], device='cuda:0', dtype=torch.int32), max_seqlen_in_batch:464
            # print(f"++++++++,input_ids_rmpad:{input_ids_rmpad}, indices:{indices}, cu_seqlens:{cu_seqlens}, max_seqlen_in_batch:{max_seqlen_in_batch}")
            # input_ids_rmpad.shape:torch.Size([845, 1]), batch_size:2, sequence_length:2560=input_len+prompt_len
            # print(f"======>>>input_ids_rmpad.shape:{input_ids_rmpad.shape}, batch_size:{batch_size}, sequence_length:{sequence_length}")
...
class ParallelDeepseekV2ForValueRmPadPP(ParallelDeepseekV2ForCausalLMRmPadPP):
